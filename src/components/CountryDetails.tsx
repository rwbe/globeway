...
export const CountryDetails: React.FC<CountryDetailsProps> = ({ country, isDarkMode, onGoBack }) => {
  ...
  const formatNumber = (num: number, style: 'decimal' | 'currency' = 'decimal', currency: string = 'USD') => {
  ...
  const formattedGdpPerCapita = formatNumber(parseFloat(gdpPerCapita), 'currency');
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={cardVariants}
        className={`${isDarkMode ? 'bg-neutral-900' : 'bg-white'} rounded-2xl p-8 shadow-lg`}
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { icon: <FaMapMarkerAlt className="w-6 h-6" />, title: t('continent'), value: country.continents.join(', ') },
            { icon: <FaClock className="w-6 h-6" />, title: t('timeZones'), value: country.timezones.join(', ') },
            { icon: <FaLanguage className="w-6 h-6" />, title: t('languages'), value: Object.values(country.languages || {}).join(', ') },
            { icon: <FaDollarSign className="w-6 h-6" />, title: t('currencies'), value: Object.values(country.currencies || {}).map((c) => `${c.name} (${c.symbol})`).join(', ') },
            { icon: <FaPhone className="w-6 h-6" />, title: t('callingCode'), value: `${country.idd.root}${country.idd.suffixes?.[0]}` },
            { icon: <FaGlobe className="w-6 h-6" />, title: t('domain'), value: country.tld?.[0] || 'N/A' },
            { icon: <FaCar className="w-6 h-6" />, title: t('drivingSide'), value: country.car.side },
            { icon: <FaMap className="w-6 h-6" />, title: t('region'), value: country.region },
            { icon: <MdNavigation className="w-6 h-6" />, title: t('latitudeLongitude'), value: `${country.latlng[0]}, ${country.latlng[1]}` },
            { icon: <FaCalendarAlt className="w-6 h-6" />, title: t('startOfWeek'), value: country.startOfWeek },
            { icon: <FaBookOpen className="w-6 h-6" />, title: t('nativeName'), value: Object.values(country.name.nativeName || {}).map((n) => n.common).join(', ') },
            { icon: <FaSmile className="w-6 h-6" />, title: t('demonym'), value: country.demonyms?.eng?.m || 'N/A' },
          ].map((item, index) => (
